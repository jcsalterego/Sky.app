...
function getElementsWithMaxScrollHeight() {
    ...
    fs = ary
        .filter((item) => item.scrollHeight === maxScrollHeight)
        .map((item) => item.elem);
    return fs;
}
function pageUpDownEventHandler(ev) {
    ...
    if (ev.target === document.body && KEYDOWN_KEYS.indexOf(ev.key) > -1) {
        ev.preventDefault();
        ...
        if (
            ev.key === "PageDown" ||
            (ev.key === " " && ev.shiftKey === false)
        ) {
            fs.forEach((f) => (f.scrollTop += f.clientHeight));
        } else if (
            ev.key === "PageUp" ||
            (ev.key === " " && ev.shiftKey === true)
        ) {
            fs.forEach((f) => (f.scrollTop -= f.clientHeight));
